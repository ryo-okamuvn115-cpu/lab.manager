  ...
  return (
    <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
                ...
                filteredItems.map((item) => {
                  ...
                  return (
                    <tr
                      ...
                      className="cursor-pointer align-top text-slate-700 transition hover:bg-slate-50"
                    >
                        ...
                        <div className="flex items-start gap-2">
                          <div className="min-w-0 flex-1">
                            <button
                              ...
                              className="break-words text-left font-medium text-slate-900 underline-offset-2 transition hover:text-blue-700 hover:underline"
                            >
                              {item.name}
                            </button>
                            <div className="mt-1 text-xs text-blue-600">タップで詳細を見る</div>
      ...
      <Modal
        ...
        title={selectedItem ? selectedItem.name : '試薬の詳細'}
        description="一覧からタップした試薬の詳細です。ここでは閲覧だけ行い、変更は編集画面から行えます。"
        footer={
          selectedItem ? (
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="text-xs font-medium text-slate-500">カテゴリ</div>
                <div className="mt-2 text-sm font-semibold text-slate-900">
                  {INVENTORY_CATEGORY_LABELS[selectedItem.category]}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="text-xs font-medium text-slate-500">発注元</div>
                <div className="mt-2 text-sm font-semibold text-slate-900">
                  {INVENTORY_SUPPLIER_LABELS[selectedItem.supplier]}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="text-xs font-medium text-slate-500">在庫数</div>
                <div className="mt-2 text-sm font-semibold text-slate-900">
                  {selectedItem.quantity} {selectedItem.unit}
                </div>
                <div className="mt-1 text-xs text-slate-500">
                  最低在庫 {selectedItem.minQuantity} {selectedItem.unit}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="text-xs font-medium text-slate-500">使用期限</div>
                <div className="mt-2 text-sm font-semibold text-slate-900">
                  {formatDate(selectedItem.expiryDate)}
                </div>
                <div className="mt-1 text-xs text-slate-500">{formatRelativeDays(selectedItem.expiryDate)}</div>
              </div>
            </div>

            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="text-xs font-medium text-slate-500">保管場所</div>
              <div className="mt-2 text-sm font-semibold text-slate-900">
                {selectedItem.location || '未設定'}
              </div>
            </div>
            ...
            {selectedItem.locationImageUrl ? (
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="text-xs font-medium text-slate-500">保管場所画像</div>
                <div className="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                  <img
                    src={selectedItem.locationImageUrl}
                    alt={`${selectedItem.name} の保管場所`}
